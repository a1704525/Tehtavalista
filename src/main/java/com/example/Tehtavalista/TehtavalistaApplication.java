...
public class TehtavalistaApplication {
	...
	/*
	 * @Bean public CommandLineRunner taskDemo(TaskRepository trepository,
	 * PersonRepository prepository, UserRepository urepository) { return (args)
	 * -> { log.info("saving tasks");
	 * 
	 * prepository.save(new Person("")); prepository.save(new Person("Äiti"));
	 * prepository.save(new Person("Lydia")); prepository.save(new
	 * Person("Sofia")); prepository.save(new Person("Isi"));
	 * 
	 * trepository.save(new Task("Ohjelmoinnin tehtävät", null,
	 * prepository.findByName("Äiti").get(0))); trepository.save(new
	 * Task("Koris", null, prepository.findByName("Lydia").get(0)));
	 * trepository.save(new Task("Luistelu",
	 * null,prepository.findByName("Sofia").get(0)));
	 * 
